...
class EarningReportController extends Controller
{
    ...
    public function getBusinessEarningReport(Request $request)
    {
        ...
        $chart_data = ['net_profit' => array(), 'total_earning' => array(), 'commission_earning' => array(), 'fee_earning' => array(), 'timeline' => array()];
        if ($deterministic == 'month') {
            ...
            foreach ($months as $month) {
                ...
                foreach ($all_earnings as $key => $item) {
                    if ($item['month'] == $month) {
                        $chart_data['net_profit'][] = with_decimal_point(($item['admin_commission'] + $item['fee']) - ($item['discount_by_admin'] + $item['coupon_discount_by_admin'] + $item['campaign_discount_by_admin']));
                        ...
                        $chart_data['fee_earning'][] = with_decimal_point($item['fee']);
                        $found = 1;
                    }
                }
                if (!$found) {
                    ...
                    $chart_data['fee_earning'][] = with_decimal_point(0);
                }
            }
