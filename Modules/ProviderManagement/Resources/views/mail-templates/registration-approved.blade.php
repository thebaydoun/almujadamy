...
<head>
    ...
    <title>{{translate('Registration Approved')}}</title>
...
<div class="box">
    <div class="top-wrap-box">
        ...
        <h3 style="margin-top: 20px">{{translate('Registration Approved')}} !</h3>
        ...
        @if($emailVerification && !$provider->owner->is_email_verified)
            <p>{{translate('Your registration request has been approved by admin. You have to complete your verification process to continue with your account without any problem')}}
                .</p>
            <p>{{translate('Please click here to complete your verification Process')}}</p>
            <button class="btn"><a href="{{route('provider.auth.verification.index')}}">{{translate('Verify')}}</a></button>
        @else
            <p>{{translate('Your registration request has been approved by admin. Now you can login as a provider and view all the options')}}.</p>
        @endif
