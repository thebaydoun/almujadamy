...
class ProviderVendorController extends Controller
{

    private Variation $variation;
    private Zone $zone;
    private Vendor $category;

    public function __construct(Vendor $category, Zone $zone, Variation $variation)
    {
    ...
    public function create(Request $request): View|Factory|Application
    {
        ...
        return view('vendormodule::provider.create', compact('categories', 'zones', 'search', 'status'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $category->image = file_uploader('vendor/', 'png', $request->file('image'));
        $category->save();
        ...
        foreach ($request->lang as $index => $key) {
            if ($defaultLanguage == $key && !($request->name[$index])) {
                if ($key != 'default') {
                    $data[] = array(
                        'translationable_type' => 'Modules\VendorModule\Entities\Vendor',
    ...
    public function edit(string $id): View|Factory|Application|RedirectResponse
    {
        ...
        if (isset($category)) {
            $zones = $this->zone->where('is_active', 1)->withoutGlobalScope('translate')->get();
            return view('vendormodule::provider.edit', compact('category', 'zones'));
        }
